...
export default defineComponent({
  setup() {
    ...
    function addTodo() {
      axios.post(`${process.env.VUE_APP_URL}/store-todo`, {
        text: state.todoText
      }).then(() => {
        state.todoText = "";
        getTodos();
    ...
    function markAsDone(todo) {
      axios.post(`${process.env.VUE_APP_URL}/todo/mark-as-done`, {
        id: todo.id
      }).then(() => getTodos());
    }

    function markAsUndone(todo) {
      axios.post(`${process.env.VUE_APP_URL}/todo/mark-as-undone`, {
        id: todo.id
      }).then(() => getTodos());
    }
    ...
    function removeTodo(todo) {
      ...
      axios.post(`${process.env.VUE_APP_URL}/delete-todo`, {
        id: todo.id
      }).then(() => getTodos());
    }

    function getTodos() {
      axios.get(`${process.env.VUE_APP_URL}/todos`)
        .then(response => {
